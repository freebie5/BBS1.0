				...
				<div id="activities">
					<s:iterator value="topices" id="sts">
						<div class="activity clearfix Blog">
							主题：
							<a href="User-queryTopic?topic.id=<s:property value='#sts.id'/>">
								<s:property value="#sts.name"/>
							</a>
							<br/>
							<a onclick="javascript:return deleteConfirm()" href="Topic-deleteTopicInfo?deleteTopicInfoRequest.userid=${sessionScope.loginUserInfo.id}&deleteTopicInfoRequest.topicid=<s:property value='#sts.id' />">
								删除
							</a>
							<div class="activity_time"><s:property value="#sts.createtime"/></div>
						</div>
					</s:iterator>
				</div>
			</div>
